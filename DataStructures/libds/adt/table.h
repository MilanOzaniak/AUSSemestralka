...
namespace ds::adt {
    ...
    {
        BSTNodeType* block = nullptr;
        if (!this->tryFindNodeWithKey(key, block)) {
            return false;
        }
    ...
    void GeneralBinarySearchTree<K, T, ItemType>::removeNode(BSTNodeType* node)
    {
        ...
        case 2:
        {
            BSTNodeType* predchodcaInOrder = node->left_;
            while (this->getHierarchy()->hasRightSon(*predchodcaInOrder))
            {
                predchodcaInOrder = predchodcaInOrder->right_;
            }
            std::swap(node->data_, predchodcaInOrder->data_);
            this->removeNode(predchodcaInOrder);
        }
        break;
        }
    }
    ...
    bool GeneralBinarySearchTree<K, T, ItemType>::tryFindNodeWithKey(const K& key, BSTNodeType*& node) const
    {
        ...
        while (key != node->data_.key_ && !hierarchy->isLeaf(*node)) {
    ...
    void GeneralBinarySearchTree<K, T, ItemType>::rotateLeft(BSTNodeType* node)
    {
        ...
        BSTNodeType* parent = static_cast<BSTNodeType*>(node->parent_);
        BSTNodeType* foreFather = static_cast<BSTNodeType*>(parent->parent_);
        this->getHierarchy()->changeRightSon(*parent, nullptr);
        this->getHierarchy()->changeLeftSon(*node, nullptr);
        if (foreFather != nullptr)
        {
            if (foreFather->left_ == parent)
            {
                this->getHierarchy()->changeLeftSon(*foreFather, node);
            }
            else
            {
                this->getHierarchy()->changeRightSon(*foreFather, node);
            }
        }
        else
        {
    ...
    {
        node->data_.priority_ = std::numeric_limits<int>::lowest();
